...
min_circular_obs_radius_ (0.3),
max_circular_obs_radius_ (1.0),
obstacles_info_ (),
...
pcl::OBSTACLE_INCLUSION<PointT>::randomObstacles ()
{
	...
	for (int obs_idx=0; obs_idx < obs_count_; obs_idx++)
	{
		...
		if(rand_obs_type == 0)
		{
			...
			float circle_center_x = (end_x - start_x)*(rand()/((float)(RAND_MAX))) + start_x;
			float circle_center_y = (end_y - start_y)*(rand()/((float)(RAND_MAX))) + start_y;
			...
			addCircularObstacle(circle_center_x, circle_center_y, circle_rand_radius, rand_obs_type);
			obstacles_info_.row(obs_idx) = Eigen::Vector4f(circle_center_x, circle_center_y, circle_rand_radius, rand_obs_type);
		} else if(rand_obs_type == 1)
		{
			...
			rand_obs_type = rand()%2;
			if (rand_obs_type == 0)
			{
				rand_obs_type = -1;
			}

			std::cout << "adding circle with center:(" <<circle_center_x <<","<< circle_center_y <<"), radius:"<< circle_rand_radius << " type:" << rand_obs_type << std::endl;
			addCircularObstacle(circle_center_x, circle_center_y, circle_rand_radius, rand_obs_type);
			obstacles_info_.row(obs_idx) = Eigen::Vector4f(circle_center_x, circle_center_y, circle_rand_radius, rand_obs_type);
		}
	}
...
pcl::OBSTACLE_INCLUSION<PointT>::samePairObstacles ()
{
	...
	if(rand_obs_type == 0)
	{
		...
		std::cout << "adding circle with center:(" <<circle_center_x <<","<< circle_center_y <<") and radius:"<< circle_rand_radius << std::endl;
		addCircularObstacle(circle_center_x, circle_center_y, circle_rand_radius, rand_obs_type);
		addCircularObstacle(circle_center_x2, circle_center_y2, circle_rand_radius2, rand_obs_type);
		obstacles_info_.row(0) = Eigen::Vector4f(circle_center_x, circle_center_y, circle_rand_radius, rand_obs_type);
		obstacles_info_.row(1) = Eigen::Vector4f(circle_center_x2, circle_center_y2, circle_rand_radius2, rand_obs_type);
	} else if(rand_obs_type == 1)
	{
		/*
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointT> void
pcl::OBSTACLE_INCLUSION<PointT>::addCircularObstacle (float center_x, float center_y, float radius, int obs_type)
{
...
pcl::OBSTACLE_INCLUSION<PointT>::preProcessing ()
{
	...
	obstacles_info_ = Eigen::MatrixXf(obs_count_,4);
