function [ theta,rho ] = ransacForSameTheta(pts,iterNum,thDist,thInlrRatio, theta )
...
for p = 1:iterNum
    ...
    plot(pts(1,:),pts(2,:),'b*')
    hold on
    plot(ptSample(1,:),ptSample(2,:),'r*')
    hold off
	d = ptSample(:,2)-ptSample(:,1);
    
%     points_theta = -atan2(d(2),d(1))*180/pi;
%     if (points_theta ~= (theta-90))
%         continue;
%     end
