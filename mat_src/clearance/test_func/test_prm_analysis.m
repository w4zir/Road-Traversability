...
% script to test prm analysis function
...
edge_weights = ones(size(neighbor_idx));
node_weights = floor(rand(total_config,1)*100);
...
p = plot(G_prm,'XData',xCord(:),'yData',yCord(:),'NodeLabel',node_weights,'LineWidth',2,'MarkerSize', 6);
...
[costMat minCutNodes]= PRMAnalysisSparseWeighted(A,startNodes,goalNodes,total_config,node_weights(valid_nodes),valid_nodes,invalid_nodes);
