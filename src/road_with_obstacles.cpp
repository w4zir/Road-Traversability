...
int main(int argc, char *argv[])
{
	...
	if (argc < 4)
	{
		cerr << "usage: " << argv[0] << "input file, output file name, obstacle count, start_index, end index." << endl;
		exit (EXIT_FAILURE);
	}
	int obstacle_count;
	int s_index;
	...
	std::istringstream (argv[4]) >> s_index;
	std::istringstream (argv[5]) >> e_index;
	...
	inStream << "/home/az/git_repos/phd/road-traversability/data/pointclouds/"<<inFile.c_str();
...
//		obj.samePairObstacles();
		...
		opStream << "/home/az/git_repos/phd/road-traversability/data/pointclouds/clearance3/" << opFile.substr(0,opFile.find(".pcd")) << i <<".pcd";
		...
		saveObs << "/home/az/git_repos/phd/road-traversability/data/pointclouds/clearance3/" << opFile.substr(0,opFile.find(".pcd")) << i <<"_obs";
		std::ofstream saveObsFile(saveObs.str().c_str());
		if (saveObsFile.is_open())
		{
			for (int i=0; i < obstacle_info.rows(); i++)
			{
				saveObsFile <<obstacle_info.row(i) <<"\n";
//				saveObsFile <<obstacle_info.row(i) <<" " << -1 <<"\n";
		...
		std::cout<< saveObs.str() << " written succesfully."<<std::endl;
	}
