function [ safetyLog ] = vehicleSpecificClearance( input_args )
%Compute clearance of road patch for specific vehicle using mincut nodes
...
safetyLog = [];
for frame_id=6:frame_count
    for v_id = 2:4
        vehicle = vehicles(v_id,:);
        run('parameters.m')
        frame_id
        fprintf ('file_id: %d and v_id: %d \n', frame_id,v_id)
        
        % get frame configs
        idx = find(configsLog(:,7) == frame_id & configsLog(:,8) == v_id);
        configs = configsLog(idx,1:6);
        
        % get frame safe mincut nodes
        idx = find(safeMincutLog(:,2) == frame_id & safeMincutLog(:,3) == v_id);
        mc_idx = safeMincutLog(idx,1);
        mc_configs = configs(mc_idx,:);
        
        % get adjacency data
        idx = find(adjacencyLog(:,4) == frame_id & adjacencyLog(:,5) == v_id);
        ...
        % get obstacle info
        idx = find(obstacleLog(:,5) == frame_id & obstacleLog(:,6) == v_id);
        ...
        plotVehicle(projectedPoints(row,1),projectedPoints(row,2),90+slope*180/pi,vehicle);
        plotVehicle(projectedPoints(col,1),projectedPoints(col,2),90+slope*180/pi,vehicle);
